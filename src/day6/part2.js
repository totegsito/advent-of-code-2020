...
const resolve = (input) => input.reduce((acc, curr) => {
    ...
    for(const line of lines) {
        ...
        for (const letter of letters) {
            if (lettersMap.has(letter)) {
                const letterInMap = lettersMap.get(letter);
                lettersMap.set(letter, letterInMap + 1);
            } else {
                lettersMap.set(letter, 1);
            }
        }
    }
    for(const [_, value] of lettersMap) {
        if(value === lines.length) counter += 1;
    }
    return acc + counter
}, 0)
