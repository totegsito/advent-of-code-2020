...
const testCase =`pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
const parseInput = input => {
    ...
    return lines.map(line => {
        const fields = line.match(/(\w{3}):([^ \n\s]+)/g);
        const fieldsArray = []
        for (let i = 0; i < fields.length; i += 1) {
            const [field, value] = fields[i].split(':');
...
const validations = {
    ...
    hgt: (hgt) => {
        const extraction = hgt.match(/(\d+)((cm)|(in))$/)
        if (extraction) {
            const [_, number, measure] = extraction
...
const resolve = (input) => {
    ...
    return input.reduce((acc, curr) => {
        const hasEveryField = mandatoryFields.every(field => {
            const toValidate = curr.find(member => member.hasOwnProperty(field));
            console.table({ curr, field, toValidate })
            if (toValidate) {
                return validations[field](toValidate[field])
            }
            return false;
        })
        if (hasEveryField) return acc + 1;
        return acc
    }, 0)
}
