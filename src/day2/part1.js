...
const phraseRegex = /(\d+)-(\d+) (\w): (\w+)/i

const parseInput = (inputText) => inputText.trim()
    .split('\n')
    .map(el => {
        const [_, min, max, letter, phrase] = el.match(phraseRegex);
        return {
            min,
            max,
            letter,
...
const resolve = (input) => {
    ...
    const isValidPassword = (pass) => {
        const { min, max, letter, phrase } = pass
        const occurrences = phrase.split('').filter(el => letter === el);
        console.log({ min, max, occurrences })
        return occurrences.length >= min && occurrences.length <= max;
    }
    return newArray.filter(isValidPassword).length
}

const myInput = parseInput(input);
const output = resolve(myInput);
