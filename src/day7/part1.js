const inputCase = `light chartreuse bags contain 1 mirrored yellow bag, 2 vibrant violet bags.
...
const testCase = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
const parseInput = (input) => {
    const bagsRegex = /([\w ]+) bags contain (\d.+)\./g;
    let results = bagsRegex.exec(input);
    const containers = {};
    while(results !== null) {
        const [_, containerColor, subContainers] = results;
        containers[containerColor] = {
            used: false,
            childrens: subContainers.split(', ').reduce((acc, current) => {
                const [_, quantity, color] = current.match(/([1-9]+) ([a-z]+ [a-z]+) bags?/);
                return [...acc, color];
            }, []),
        }
        results = bagsRegex.exec(input);
    }
    return containers;
}
...
const resolve = (input) => {
    ...
    while (targetColors.length !== 0) {
        const remainingKeys = keys.filter(key => !graph[key].used)
        let newColorsToFind = [];
        for(const key of remainingKeys) {
            const current = graph[key];
            const target = current.childrens.find(color => targetColors.includes(color));
            if (target) {
                current.used = true;
                cont += 1;
                newColorsToFind = [...newColorsToFind, key];
            }
        }
        targetColors = newColorsToFind;
    }
    return cont;
};

const testInput = parseInput(testCase);
const testSolution = resolve(testInput);

console.assert(testSolution === 4);

const input = parseInput(inputCase);
const solution = resolve(input);
