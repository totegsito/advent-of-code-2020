...
const testCase = `shiny gold bags contain 2 dark red bags.
    ...
    while(results !== null) {
        ...
        containers[containerColor] = {
            ...
            childrens: subContainers.split(', ').reduce((acc, current) => {
                ...
                return [...acc, { quantity: parseInt(quantity, 10), color }];
            }, []),
        }
...
const resolve = (input) => {
    ...
    const getQuantity = (key) => {
        ...
        const { childrens } = graph[key];
        let quantity = 0;
        for(const children of childrens) {
            const { color, quantity: childrenQuantity } = children;
            quantity += childrenQuantity + childrenQuantity * getQuantity(color);
        }
        console.log(quantity)
        return quantity;
    }
    return getQuantity('shiny gold');
};

const testInput = parseInput(testCase);
const testSolution = resolve(testInput);

console.assert(testSolution === 126);
